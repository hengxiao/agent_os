...
def test_shell_exec_timeout_clamped_to_spec_no_orphan(monkeypatch):
    ...
    async def main():
        ...
        result = await reg.dispatch(
            ...
            ToolCall(id="s", name="system.shell.exec", args={"command": "sleep 61 | cat", "timeout": 120}),
            ctx,
        )
...
def _read(text: str, **args) -> str:
    ...
    async def main():
        wr = await reg.dispatch(
            ToolCall(id="w", name="system.file.write", args={"path": "f.txt", "content": text}), ctx
        )
        assert wr.ok, wr.error
        return await reg.dispatch(
            ToolCall(id="r", name="system.file.read", args={"path": "f.txt", **args}), ctx
        )
...
def test_fs_read_marks_truncation_with_total_and_next_offset():
    ...
    assert out.splitlines()[-1] == "[已显示 1-200 行,共 5000 行;续读 system.file.read(offset=201)]"
