...
| G2 契约 | inputs/outputs 是合法 JSON Schema;L2+ 每个参数有 `type`;inputs 示例骨架可生成;**引用完整性两阶段(docs/SKILL-PACKAGES-V2.md §6.2):草稿期 dangling = warn + 修复提示,提交期(strict_refs)= fail** | 契约冻结面 |
