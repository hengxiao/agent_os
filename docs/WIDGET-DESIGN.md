...
> **v2 · 用户验收反馈(2026-08-05)**:card 重设计——**撤掉首行原文预览**
> (「{」没有信息量),改为:状态行(✓ 合法 / ✕ 第 N 行错误,同 v1)+
> **顶层键名 chips**(前 4 个,溢出 +N,mono)+ meta 行(N 键 · 大小 ·
> 相对时间)。错误态左边条 `--danger` 保留。已实施(渲染/断言/效果图)。
...
> **v2 · 用户验收反馈(2026-08-05)**:被批「设计太复杂」,整体简化——
> - 单元格改 **KV 式常驻可编辑 input**(隐形边框,focus 才显),废除「点击
>   进编辑模式」的态切换;enum 列 = 隐形样式原生 select,boolean = checkbox;
> - 键盘 Excel 逻辑:Tab/Shift+Tab 横向走格、Enter 下移、左右方向键在
>   光标位于端点时跨格(checkbox/select 无光标直跨);
> - **行 DnD 全撤**(拖柄/指示线/dragging 浮起/envelope 一并退役;
>   Alt+↑/↓ 键盘移行保留);行尾 ✕ hover 显保留;「+ 添加行」虚线行保留;
> - 列头保留(名称+类型图标+必填 *),**排序/⋯ 视觉槽撤掉**;sticky 保持;
> - card 不变。下列验收清单替换 v1。

**验收清单(v2)**:
- [x] 单元格常驻可编辑,零模式切换(值随 input/change 即时同步);
- [x] Excel 走格:Tab/Shift+Tab 横向、Enter 下移、←/→ 端点跨格;
- [x] 行 DnD/envelope 已退役(无 draggable、无 envelope、无拖拽监听;
      Alt+↑/↓ 移行保留);
...
> **v2 · 用户验收反馈(2026-08-05)**:排版修复——**单列为主**(Stripe 式
> 整齐排版;W6.2 的两列网格撤掉,字段高不齐显乱);字段根内部顺序钉死:
> label 上置(12px/500)→ 控件(同高 32)→ 帮助文字 → 错误行(F4 的
> 根内收拢语义并入正文,有「排版结构断言」盯)。嵌套 object 卡片化分组、
> 操作行贴底(reset ghost 居左 dirty 才可用 + live 圆点 + 主操作实心居右)
> 不变。
...
> **v2 · 用户验收反馈(2026-08-05)**:紧凑化(文件管理器左侧树)——
> 行高 24–26px、缩进 12px/级、chevron 10px;行内只留 图标(▸/▾)+ 名称
> + 计数胶囊(弱);去掉多余 meta。hover --bg-2 / 选中 --live 左条+浅底 /
> 过滤调光 / 键盘操作全部保留。
...
> **v2 · 用户验收反馈(2026-08-05;真实浏览器验收「完全不能使用」)——
> 根因 → 修法三条**:
> 1. **输入逐字重渲丢焦点/光标**(set() 每击键整树重渲,输入框被替换)→
>    set() 改走 `preserveSelection`(W5.1 基座同答案,定位面 = 对应输入框
>    的 data 属性选择器),有断言;
> 2. **点外收层误伤弹层内点击**(pick/翻月整树重渲后,bubble 阶段的
>    document 委托收到 click 时目标元素已脱离文档,`contains` 误判为外
>    → 弹层一点就关)→ 委托改 **capture 阶段**(在宿主重渲之前执行);
> 3. **右缘出视口不可点**(弹层只做了下溢上翻)→ `_flipLayer` 加横向:
>    右出视口则贴宿主右边(`.wd-date-layer.right`)。
> 验收三条(v1)仍成立且已勾;本 v2 为真实环境可用性修复。
...
> **v2 · 用户验收反馈(2026-08-05)**:新增 **view source** 选择——segmented
> 「预览 | 源码」;`state.view ∈ {preview, source}`,切换不重取数据;
> 源码态 = mono 只读视图(复用 W-text readonly 视觉:行号槽 + 微标),
> 内容为原始 markdown;**全文复制钮两态都在**(预览态保留代码块逐个复制)。
...
> **v3 实现注**(2026-08-12;**用户裁决,覆盖 v2「点外不收起」**):
> - **原位开泡**:右键在块内任意点 → 壳浮在**该点旁**(point 块内偏移
>   offTop/offLeft 进 entry;无选区时 anchor = 点击点所在行级块);
> - **选区关联**:块内有选区时右键 → anchor 升行列范围
>   `doc.md#L7:C3-L9:C10`(列可选,旧行级全兼容;前后端 `_ANCHOR_RE` 同步
>   升级;apply 仍按整行段替换,列级精确定位未开);quote = 选中文本;
>   标记语法块(#/-/| 前缀)渲染文与源有符号差,列映射回落行级;
> - **点泡外收起**(覆盖 v2):壳藏起 + 💬 标记留在**原位附近**(锚点行尾,
>   offTop 驱动);点标记原位重开,草稿/canonical state 逐字在(收起≠删除);
>   注意实现坑:泡内控件点击先重渲卡面把事件目标摘出 DOM,宿主冒泡收到时
>   closest 已空——点外判定须按控件 data 面(data-bubble-* 白名单)判内;
> - **垃圾桶删除**:卡头 🗑(两击确认)→ widget `delete` action
>   (exec:local——协议原则 action 必是 skill)→ 事件上行 → 宿主摘除壳/标记
>   + `remove_child(destroy)` + `POST /api/docs/<name>/bubbles/delete`
>   (doc_store.delete_bubble 删持久化;**幂等**:无流也 200 deleted=false);
>   壳内原 ✕ fold 钮并入卡面头部(叠位冲突消除);
> - 壳/标记的块归属按**块级锚点**(blockAnchor;列范围锚点不对应块),
>   `_rehangShells`/`_fitBubble` 均走 blockAnchor + offTop;
> - **测试**:stub(parseAnchor 列范围/旧兼容/非法拒;delete action 声明;
>   垃圾桶两击上行一次)+ pytest 2 例(store 删/端点幂等与格式)+ tests-ui
>   四断言组(原位点旁/选区 quote+列范围/点外收起原位标记/重开草稿在/
>   垃圾桶后端同步)+ 三截图(.shots/bubble-v3-*)。

> **v3.1 实现注**(2026-08-12;用户裁决:批注从「行」粒度升到「行文位置」粒度):
> - **点锚点**:无选区右键经 `caretRangeFromPoint` 映射点击点行列 → 零宽锚点
>   `L3:C8-L3:C8`(quote 空串);选区维持列范围;标记语法块回落行级不变;
> - **行内高亮**:列范围锚点的文本包 `.doc-hl`(split text nodes;`_relayout`
>   内重挂,幂等防叠包);偏移计算跳过泡壳/标记子树(TreeWalker filter);
> - **标记跟随**:列范围 → 选段末矩形右侧;零宽点 → 点位旁;行级 → 行尾;
> - **一行多泡**:锚点串即 Map 键天然分立;修掉 C3 潜伏 bug——add_child 走
>   基座内层 `_relayout`(不经宿主包装),先开的壳会被摘出 DOM,openBubble
>   补 `_rehangShells()`;
> - **点高亮 = 重开**:preview 点击 `.doc-hl` → openBubble(锚点);点外收起
>   判定把 `.doc-hl` 列入"泡内"白名单(否则开完即被收);
> - **测试**:stub(点锚点解析往返/anchorColOffsetsOf 纯函数四界/同行多锚点
>   分立)+ tests-ui 五断言组(一行两泡两标记/高亮文本=选中文本/标记跟选段末/
>   点高亮重开/重渲后高亮仍在)+ 三截图(.shots/bubble-v31-*)。

> **v3.2 实现注**(2026-08-13;UX 验收 F1–F4 修复):
> - **F1 点旁开泡**:壳左缘 = 点击点块内偏移,只钳 8px 边距——去掉
>   「块宽−48」右缘 clamp(那会把泡吸到行右缘呈固定面板);视口边界交给
>   Floating UI `shift({padding:16})`;纵向保持点击行下方;
> - **F2 无效坐标回落**:右键坐标 (0,0)/非有限(合成事件、过渡态)→ 按块
>   矩形中心再试 `caretRangeFromPoint`;仍不成回落行级(该行已有行级泡则
>   聚焦不新建);块零 rect(detached 旧块)→ `_fitBubble` 跳本拍 +
>   `requestAnimationFrame` 重试;重开路径先 `_rehangShells()` 挂回再定位;
> - **F3 高度封顶**:maxHeight = clamp(200px, min(380px, 45vh),
>   可用−16px)——380px 绝对上限(原 45vh 在高大屏无顶),超出 log 内滚;
> - **F4 连发不吞(补强)**:`_submit` 发送时读 composer DOM 当前值
>   (不只 state.draft——直写 value 不经 input 事件时 draft 滞后吞消息);
