...
- **if_match 乐观锁**:`system.file.write`/`system.file.edit` 接受 `if_match: str`(内容 hash),
  不匹配拒写并返回当前版本标识与"重读后重试"提示;
- **system.shell.exec 结构化返回**:`{stdout, stderr, exit_code, truncated, text}`;
...
def _ctx(workdir: Path, *, read_paths: list[Path] | None = None) -> ToolDispatchContext:
    ...
    return ToolDispatchContext(
        frame=frame,
        allowed_tools=["system.file.read", "system.file.write", "system.file.edit", "system.shell.exec"],
...
def test_default_workdir_unchanged(tmp_path):
    ...
    ctx = ToolDispatchContext(
        ...
        allowed_tools=["system.file.write"],
        tool_policy=ToolPolicy(max_permission=Permission.WRITE),
    )

    async def main():
        wr = await reg.dispatch(ToolCall(id="w", name="system.file.write", args={"path": "a.txt", "content": "x"}), ctx)
...
def test_configured_workdir_resolves_inside(tmp_path):
    ...
    async def main():
        wr = await reg.dispatch(ToolCall(id="w", name="system.file.write", args={"path": "proj/a.txt", "content": "hi"}), ctx)
        assert wr.ok, wr.error
        assert (tmp_path / "proj" / "a.txt").read_text() == "hi"
        sh = await reg.dispatch(ToolCall(id="s", name="system.shell.exec", args={"command": "cat proj/a.txt"}), ctx)
        return sh
...
def test_readonly_zone_rejects_writes(tmp_path):
    ...
    async def main():
        rd = await reg.dispatch(
            ToolCall(id="r", name="system.file.read", args={"path": str(ro / "lib.py")}), ctx
        )
        assert rd.ok, rd.error
        wr = await reg.dispatch(
            ToolCall(id="w", name="system.file.write", args={"path": str(ro / "evil.py"), "content": "x"}), ctx
        )
        return wr
...
def test_readonly_zone_outside_workdir_still_readable(tmp_path):
    ...
    async def main():
        ok = await reg.dispatch(ToolCall(id="r", name="system.file.read", args={"path": str(ro / "lib.py")}), ctx)
        bad = await reg.dispatch(
            ToolCall(id="r2", name="system.file.read", args={"path": "/etc/hostname"}), ctx
        )
        return ok, bad
...
def test_system_file_read_missing_file_has_actionable_hint(tmp_path):
    reg = LocalPythonToolRegistry.with_builtins()
    ctx = _ctx(tmp_path)

    async def main():
        return await reg.dispatch(ToolCall(id="r", name="system.file.read", args={"path": "ghost.txt"}), ctx)
...
def test_path_escape_has_actionable_hint(tmp_path):
    ...
    async def main():
        return await reg.dispatch(ToolCall(id="r", name="system.file.read", args={"path": "../x.txt"}), ctx)
...
def test_system_file_write_if_match_optimistic_lock(tmp_path):
    reg = LocalPythonToolRegistry.with_builtins()
    ctx = _ctx(tmp_path)

    async def main():
        await reg.dispatch(ToolCall(id="w1", name="system.file.write", args={"path": "a.txt", "content": "v1"}), ctx)
        # 正确 if_match(当前内容 hash)→ 成功
        ok = await reg.dispatch(
            ToolCall(id="w2", name="system.file.write", args={"path": "a.txt", "content": "v2", "if_match": "v1"}), ctx
        )
        assert ok.ok, ok.error
        # 过期 if_match → 拒写
        stale = await reg.dispatch(
            ToolCall(id="w3", name="system.file.write", args={"path": "a.txt", "content": "v3", "if_match": "v1"}), ctx
        )
        return stale
...
# ---------------------------------------------------------------------------
# W0-5 system.shell.exec 结构化返回
# ---------------------------------------------------------------------------


def test_system_shell_exec_structured_result(tmp_path):
    reg = LocalPythonToolRegistry.with_builtins()
    ctx = _ctx(tmp_path)

    async def main():
        ok = await reg.dispatch(
            ToolCall(id="s1", name="system.shell.exec", args={"command": "echo out && echo err >&2"}), ctx
        )
        fail = await reg.dispatch(ToolCall(id="s2", name="system.shell.exec", args={"command": "exit 3"}), ctx)
        return ok, fail
...
def test_read_tools_declare_contract_fields():
    reg = LocalPythonToolRegistry.with_builtins()
    system_file_read = reg.get("system.file.read").spec
    assert system_file_read.cacheable is True
    assert system_file_read.concurrent_safe is True
    assert system_file_read.idempotent is True
    assert hasattr(system_file_read, "cost_hint"), "ToolSpec 需要 cost_hint 字段"
