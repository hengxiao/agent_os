...
锚点已覆盖五技能的 happy path 与 common.dev.apply_patch 冲突拒绝,本文件补薄弱处:

- common.eval.progress_track:init 覆盖拒绝/重复 id/空 tasks、update 未知 id/非布尔 passes、
  summary 文件不存在(NOT_FOUND)/坏 JSON、路径越界拒绝;
- common.dev.run_tests:退出码 5(no tests collected)绝不当绿灯、收集错误计入 errors、
  路径不存在 NOT_FOUND;
- common.dev.read_file_smart:非法 level、NOT_FOUND、L2 分页边界;
- common.dev.apply_patch:新建/删除/多文件原子性/行号偏移唯一补偿/多义拒绝/畸形与
  计数不符拒绝/越界拒绝;
- common.dev.summarize_tree:深度上限、.gitignore 与 .git 跳过、关键文件、非目录拒绝;
- 确定性:同输入同输出(common.dev.read_file_smart / common.dev.summarize_tree)。
"""
...
def test_init_refuses_overwrite_and_bad_tasks(tmp_path):
    ...
    with pytest.raises(HANDLER_ERRORS, match="已存在"):
        run("common.eval.progress_track", {"action": "init", "path": path, "tasks": [{"id": "t2", "text": "y"}]}, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="重复"):
        run("common.eval.progress_track", {
            "action": "init", "path": str(tmp_path / "q.json"),
            "tasks": [{"id": "t", "text": "a"}, {"id": "t", "text": "b"}],
        }, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="非空"):
        run("common.eval.progress_track", {"action": "init", "path": str(tmp_path / "r.json"), "tasks": []}, tmp_path)
...
def test_update_unknown_task_and_non_bool_passes(tmp_path):
    ...
    with pytest.raises(HANDLER_ERRORS, match="任务不存在"):
        run("common.eval.progress_track", {"action": "update", "path": path, "task_id": "nope", "passes": True}, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="boolean"):  # inputs schema 层拦截
        run("common.eval.progress_track", {"action": "update", "path": path, "task_id": "t1", "passes": "yes"}, tmp_path)


def test_summary_missing_file_bad_json_and_order(tmp_path):
    with pytest.raises(HANDLER_ERRORS, match="NOT_FOUND"):
        run("common.eval.progress_track", {"action": "summary", "path": "missing.json"}, tmp_path)
    bad = tmp_path / "bad.json"
    bad.write_text("not json", encoding="utf-8")
    with pytest.raises(HANDLER_ERRORS, match="合法 JSON"):
        run("common.eval.progress_track", {"action": "summary", "path": "bad.json"}, tmp_path)
    path = str(tmp_path / "p.json")
    run("common.eval.progress_track", {
        "action": "init", "path": path,
        "tasks": [{"id": "t1", "text": "a"}, {"id": "t2", "text": "b"}, {"id": "t3", "text": "c"}],
    }, tmp_path)
    run("common.eval.progress_track", {"action": "update", "path": path, "task_id": "t2", "passes": True}, tmp_path)
...
def test_run_tests_missing_path(tmp_path):
    with pytest.raises(HANDLER_ERRORS, match="NOT_FOUND"):
        run("common.dev.run_tests", {"path": "no/such/dir"}, tmp_path)
...
def test_read_file_smart_bad_level_and_missing(tmp_path):
    ...
    with pytest.raises(HANDLER_ERRORS, match="L0"):  # enum 在 inputs schema 层拦截
        run("common.dev.read_file_smart", {"path": "a.py", "level": "L3"}, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="NOT_FOUND"):
        run("common.dev.read_file_smart", {"path": "ghost.py", "level": "L0"}, tmp_path)


def test_read_file_smart_l2_bounds(tmp_path):
    (tmp_path / "a.txt").write_text("x\ny\n", encoding="utf-8")
    beyond = run("common.dev.read_file_smart", {"path": "a.txt", "level": "L2", "offset": 10}, tmp_path)
    assert beyond["text"] == "" and beyond["total_lines"] == 2
    with pytest.raises(HANDLER_ERRORS):
        run("common.dev.read_file_smart", {"path": "a.txt", "level": "L2", "offset": 0}, tmp_path)


def test_read_file_smart_markdown_headings_and_empty_structure(tmp_path):
    (tmp_path / "doc.md").write_text("# 标题\n\n正文\n\n## 小节\n", encoding="utf-8")
    l1 = run("common.dev.read_file_smart", {"path": "doc.md", "level": "L1"}, tmp_path)
    heads = [s for s in l1["structure"] if s["kind"] == "heading"]
    assert [h["name"] for h in heads] == ["标题", "小节"]
    (tmp_path / "plain.txt").write_text("只有文本\n没有定义\n", encoding="utf-8")
    l0 = run("common.dev.read_file_smart", {"path": "plain.txt", "level": "L0"}, tmp_path)
...
def test_apply_patch_create_and_delete(tmp_path):
    create = run("common.dev.apply_patch", {"patch": (
        "--- /dev/null\n"
        "+++ b/new.txt\n"
        "@@ -0,0 +1,2 @@\n"
        "+a\n"
    ...
    # 新建目标已存在 → 冲突拒绝
    again = run("common.dev.apply_patch", {"patch": (
        "--- /dev/null\n"
        "+++ b/new.txt\n"
        "@@ -0,0 +1 @@\n"
        "+z\n"
    ...
    # 删除(hunks 覆盖整个文件)
    delete = run("common.dev.apply_patch", {"patch": (
        "--- a/new.txt\n"
        "+++ /dev/null\n"
        "@@ -1,2 +0,0 @@\n"
        "-a\n"
...
def test_apply_patch_multi_file_is_atomic(tmp_path):
    ...
    r = run("common.dev.apply_patch", {"patch": (
        "--- a/f1.txt\n"
        "+++ b/f1.txt\n"
        "@@ -1,2 +1,2 @@\n"
        " a\n"
...
def test_apply_patch_line_number_fuzz_and_ambiguity(tmp_path):
    ...
    conflict = run("common.dev.apply_patch", {"patch": (
        "--- a/d.txt\n"
        "+++ b/d.txt\n"
        "@@ -5,1 +5,1 @@\n"
        "-dup\n"
...
def test_apply_patch_multi_hunk_and_malformed(tmp_path):
    ...
    with pytest.raises(HANDLER_ERRORS, match="unified diff"):
        run("common.dev.apply_patch", {"patch": "这不是补丁"}, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="超过 @@ 声明"):
        run("common.dev.apply_patch", {"patch": (
            "--- a/n.txt\n"
            "+++ b/n.txt\n"
            "@@ -1,1 +1,1 @@\n"
            "-1\n"
...
def test_summarize_tree_depth_limit(tmp_path):
    ...
    r = run("common.dev.summarize_tree", {"path": ".", "depth": 1}, tmp_path)
    assert r["file_count"] == 1 and r["dir_count"] == 1
    assert "deep.py" not in r["summary"]
    full = run("common.dev.summarize_tree", {"path": "."}, tmp_path)
...
def test_summarize_tree_rejects_file_and_missing(tmp_path):
    ...
    with pytest.raises(HANDLER_ERRORS, match="不是目录"):
        run("common.dev.summarize_tree", {"path": "f.txt"}, tmp_path)
    with pytest.raises(HANDLER_ERRORS, match="NOT_FOUND"):
        run("common.dev.summarize_tree", {"path": "ghost"}, tmp_path)
    with pytest.raises(HANDLER_ERRORS):
        run("common.dev.summarize_tree", {"path": ".", "depth": 0}, tmp_path)
...
def test_determinism_same_input_same_output(tmp_path):
    ...
    a = run("common.dev.read_file_smart", {"path": "m.py", "level": "L1"}, tmp_path)
    b = run("common.dev.read_file_smart", {"path": "m.py", "level": "L1"}, tmp_path)
    assert a == b
    s1 = run("common.dev.summarize_tree", {"path": "."}, tmp_path)
    s2 = run("common.dev.summarize_tree", {"path": "."}, tmp_path)
    assert s1 == s2
    # progress 文件落盘内容也确定(不读钟)
    p = str(tmp_path / "p.json")
    run("common.eval.progress_track", {"action": "init", "path": p, "tasks": [{"id": "t", "text": "x"}]}, tmp_path)
    first = json.loads(Path(p).read_text(encoding="utf-8"))
    run("common.eval.progress_track", {"action": "update", "path": p, "task_id": "t", "passes": False}, tmp_path)
