...
# ═══ 3.13 W-bubble ════════════════════════════════════════════════════════
def w_bubble():
    g = page("W-bubble · 聊天气泡", "Notion comment / Slack thread")
    ...
    g.append(ps("M336 166 L348 166 L342 154 Z", BG1, LINE))
    g += shadow(280, 166, 660, 294, 12, 3)
    g.append(rs(280, 166, 660, 294, 12, BG1, LINE))
    g.append(ts(300, 192, "批注 · lab.travel / plan.md", 12, FG1, 500))
    g.append(cs(916, 188, 11, BG2))  # ✕ hover
    g.append(ts(916, 192, "✕", 11, FG1, anchor="middle"))
    # 锚点引用块
    g.append(rs(300, 204, 3, 34, 0, LIVE))
    g.append(ts(312, 220, "第三天行程:上午故宫,下午颐和园,", 12, FG1))
    g.append(ts(312, 238, "晚餐四季民福,预算 ¥720。", 12, FG1))
    p, _ = pill(888, 208, "L7", "neutral", h=18, mono=True)
    g.append(p)
    g.append(ls(300, 256.5, 924, 256.5, LINE))
    # 消息流(Notion 式:不分左右)
    g.append(cs(310, 276, 10, "#33405a"))
    g.append(ts(310, 280, "沈", 10, WHITE, 500, anchor="middle"))
    g.append(ts(328, 280, "沈航", 12, FG0, 500))
    g.append(ts(328 + tw("沈航", 12) + 8, 280, "2 小时前", 11, FG2))
    g.append(ts(328, 302, "这段预算偏高,能压到 ¥600 以内吗?", 13, FG0))
    g.append(cs(310, 326, 10, LIVE))
    g.append(ts(310, 330, "✦", 10, WHITE, anchor="middle"))
    g.append(ts(328, 330, "Agent", 12, FG0, 500))
    g.append(ts(328 + tw("Agent", 12) + 8, 330, "1 小时前", 11, FG2))
    g.append(ts(328, 352, "可以。故宫改景山(省 ¥60),晚餐改", 13, FG0))
    g.append(ts(328, 374, "四季民福(省 ¥80),合计 ¥596。", 13, FG0))
    for k in range(3):  # typing 三点
        g.append(cs(332 + k * 8, 398, 2, FG2, op=1 - k * 0.35))
    # 输入区(焦点环)+ 发送钮
    g.append(input_box(300, 412, 472, h=36, value="好的,按方案 B 更新", focused=True))
    b, _ = button(784, 412, "发送", "primary", h=36, size=13)
    g.append(b)
    # card(收起态):22px 圆标 + hover 预览条
    g += card()
    g.append(ts(38, 540, "第三天:上午故宫,下午颐和园,预算 ¥720。", 12, FG2))
    g.append(cs(342, 536, 11, LIVE))
    g.append(ts(342, 540, "2", 11, WHITE, 600, anchor="middle"))
    g += shadow(38, 560, 312, 44, 8, 2)
    g.append(rs(38, 560, 312, 44, 8, BG1, LINE))
    g.append(ts(50, 578, "Agent:改走景山方案,合计 ¥596", 12, FG0))
    g.append(ts(50, 596, "3 条 · 2 条未读", 11, FG2))
    # 状态变体:无未读 / 发送失败
