...
- `progress_track`(code):JSON 进度文件,`{action: init|update|summary}`;
  init 建文件(tasks 全 passes:false),update 置 passes,summary 给 resume 摘要;
- `run_tests`(code,EXEC):跑 pytest 并**结构化解析** `{passed, failed, errors, failures[], ok}`;
- `read_file_smart`(code):`{path, level: L0|L1|L2, offset?, limit?}` 三级阅读
  (L0 概要/L1 结构/L2 全文带行号);
- `apply_patch`(code):unified diff 应用到 workdir,`{applied, files}`;冲突拒绝;
- `summarize_tree`(code):目录级结构摘要(目录树 + 计数 + 关键文件)。
"""
...
def test_progress_track_lifecycle(tmp_path):
    ...
    r2 = run("progress_track", {"action": "update", "path": path, "task_id": "t1", "passes": True}, tmp_path)
    assert r2["ok"] is True
    data = json.loads(Path(path).read_text())
    assert next(t for t in data["tasks"] if t["id"] == "t1")["passes"] is True

    r3 = run("progress_track", {"action": "summary", "path": path}, tmp_path)
...
def test_progress_track_complete_when_all_pass(tmp_path):
    ...
    run("progress_track", {"action": "update", "path": path, "task_id": "t1", "passes": True}, tmp_path)
...
def test_read_file_smart_levels(tmp_path):
    ...
    l0 = run("read_file_smart", {"path": "big.py", "level": "L0"}, tmp_path)
    assert "30" in l0["summary"] or "f0" in l0["summary"]
    l1 = run("read_file_smart", {"path": "big.py", "level": "L1"}, tmp_path)
    assert "def f" in l1["summary"] or any("def f" in s for s in l1.get("structure", []))
    l2 = run("read_file_smart", {"path": "big.py", "level": "L2", "offset": 3, "limit": 4}, tmp_path)
...
def test_apply_patch_applies_and_conflicts(tmp_path):
    ...
    conflict = run("apply_patch", {"patch": patch}, tmp_path)
