"""工具契约(docs/DESIGN.md §2.2;§14.1 冻结清单:ToolSpec 全部预留字段、
...
class ToolErrorKind(Enum):
    ...
    DATA_ACCESS_DENIED = "data_access_denied"  # 数据层 authZ 拒绝(docs/DATA-AUTHZ.md §3.3)
    VETOED = "vetoed"
    NOT_FOUND = "not_found"
    TIMEOUT = "timeout"
...
class ToolSpec:
    ...
    # —— 升权分档(docs/ESCALATION.md §2.1;additive)——
...
class ToolContext:
    ...
    run_id: str = ""
    frame_id: str = ""
    principal: Any = None  # caller identity(docs/DATA-AUTHZ.md §2:run 的 Principal;None = 单用户语义)
